...
tf.app.flags.DEFINE_string('image_dir', '/home/matthia/Desktop/MSc.-Thesis/CNNImages/BasicEvaluation/',
                           """Path to folders of labeled images.""")
tf.app.flags.DEFINE_string('output_graph', '/home/matthia/Desktop/MSc.-Thesis/Inception/NN_param/output_graph.pb',
                           """Where to save the trained graph.""")
tf.app.flags.DEFINE_string('output_labels', './NN_param/output_labels.txt',
                           """Where to save the trained graph's labels.""")
tf.app.flags.DEFINE_string('summaries_dir', './NN_param/retrain_logs',
...
tf.app.flags.DEFINE_integer('eval_step_interval', 20,
...
tf.app.flags.DEFINE_string(
    'bottleneck_dir', './NN_param/bottleneck',
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  hiddenSize = 250;
  wh1 = tf.Variable(tf.truncated_normal([BOTTLENECK_TENSOR_SIZE, hiddenSize], stddev=0.001));
  bh1 = tf.Variable(tf.truncated_normal([hiddenSize], stddev=0.001))
  
  hOut = tf.nn.relu(tf.matmul(bottleneck_input, wh1) + bh1);
  
  layer_name = 'final_training_ops'
  with tf.name_scope(layer_name):
    with tf.name_scope('weights'):
      layer_weights = tf.Variable(tf.truncated_normal([hiddenSize, class_count], stddev=0.001), name='final_weights')
      variable_summaries(layer_weights, layer_name + '/weights')
    with tf.name_scope('biases'):
      layer_biases = tf.Variable(tf.zeros([class_count]), name='final_biases')
      variable_summaries(layer_biases, layer_name + '/biases')
    with tf.name_scope('Wx_plus_b'):
      logits = tf.matmul(hOut, layer_weights) + layer_biases
...
def main(_):
  ...
  for i in range(FLAGS.how_many_training_steps):
    ...
    if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
      ...
      cross_entropy_list.append(cross_entropy_value*10)
      ...
      if len(validation_list)>10:
        validation_av.append(np.array(validation_list[-10:]).mean())
      else:
        validation_av.append(np.array(validation_list).mean())
      np.save('val_acc', np.asarray(validation_list))
